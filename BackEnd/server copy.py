...
def delete_associazione_progetto_questionario():
    """
    Rimuove l'associazione tra un progetto e un questionario.
    Parametri JSON: id_progetto, id_questionario
    """
    db_session = SessionLocal()
    try:
        data = request.json
        ...
        if not progetto or not questionario:
            return jsonify({'error': 'Progetto o Questionario non trovati'}), 404

        if questionario not in progetto.questionari:
            return jsonify({'error': 'Associazione non esiste'}), 404

        progetto.questionari.remove(questionario)
        db_session.commit()
